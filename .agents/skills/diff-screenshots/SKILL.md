...
bun scripts/render-pack-debug.ts my-pack --out /tmp/before.png

# ... make shader edits ...

# Render after the change
bun scripts/render-pack-debug.ts my-pack --out /tmp/after.png

# Compare
bun scripts/diff-png.ts /tmp/before.png /tmp/after.png
```
...
```bash
bun scripts/render-pack-debug.ts bloom-pulse --param rings=4  --out /tmp/rings4.png
bun scripts/render-pack-debug.ts bloom-pulse --param rings=24 --out /tmp/rings24.png
bun scripts/diff-png.ts /tmp/rings4.png /tmp/rings24.png
```

### Compare presets

```bash
bun scripts/render-pack-debug.ts bloom-pulse --preset Soft    --out /tmp/soft.png
bun scripts/render-pack-debug.ts bloom-pulse --preset Inferno --out /tmp/inferno.png
bun scripts/diff-png.ts /tmp/soft.png /tmp/inferno.png
```
...
```bash
bun scripts/render-pack-debug.ts my-pack --audio bass=0   --out /tmp/no-bass.png
bun scripts/render-pack-debug.ts my-pack --audio bass=1.0 --out /tmp/full-bass.png
bun scripts/diff-png.ts /tmp/no-bass.png /tmp/full-bass.png
```
...
bun scripts/render-pack-debug.ts my-pack --capture-every 0.5 --time 3.0 --out /tmp/temporal.png --width 320 --height 240

# Diff frame at t=0s vs t=1s
bun scripts/diff-png.ts /tmp/temporal_t0.0s.png /tmp/temporal_t1.0s.png

# Diff sequential frames to check smooth animation
bun scripts/diff-png.ts /tmp/temporal_t0.0s.png /tmp/temporal_t0.5s.png
bun scripts/diff-png.ts /tmp/temporal_t0.5s.png /tmp/temporal_t1.0s.png
```
...
```bash
bun scripts/diff-png.ts /tmp/before.png /tmp/after.png --out /tmp/diff.png
...
```bash
bun scripts/diff-png.ts /tmp/run1.png /tmp/run2.png --threshold 0
```

### JSON output for scripting

```bash
result=$(bun scripts/diff-png.ts /tmp/a.png /tmp/b.png --json)
echo $result
```
...
# 2. Render baseline
bun scripts/render-pack-debug.ts my-pack --out /tmp/before.png --width 320 --height 240
...
bun scripts/render-pack-debug.ts my-pack --out /tmp/after.png --width 320 --height 240
...
| File | Role |
|------|------|
| `scripts/diff-png.ts` | The comparison script (uses pixelmatch + pngjs) |
| `scripts/render-pack-debug.ts` | Renders PNGs to compare |
